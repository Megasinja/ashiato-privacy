...
### 3-1b. 「ルート画像から座標を起こす」という発想について

「ルート検索で出た**画像**を座標に変換したい」というニーズがあるが、これは用語としては
ジオコーディングではなく**ジオリファレンス＋ベクトル化**。そして**大半のケースで不要**:
ルートAPIの応答には座標が最初から入っている（Google `overview_polyline` をデコード、
ORS は GeoJSON 座標列、Yahoo は「経路探索」でジオメトリ取得）。

画像処理が本当に要るのは「紙地図など**デジタル元データが存在しない**画像」に限る。
なお **Google 由来の座標は、画像経由でもデコードでも恒久保存は規約違反**（座標化しても
制限は外れない）。詳細・パイプライン・Web Mercator逆変換は
**`route-image-to-coordinates.md`** を参照。
